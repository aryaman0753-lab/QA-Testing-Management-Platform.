...
export function BugDetails() {
  ...
  return <AppLayout>
    ...
    {bug.discovered_from_load_test_run && <section className="panel"><h2>Load Test Source</h2><p><strong>{bug.discovered_from_load_test_run.run_key}</strong> · {bug.discovered_from_load_test_run.threshold_results.filter((item) => !item.passed).length} threshold violation(s)</p><Link to={`/projects/${projectId}/load-testing/runs/${bug.discovered_from_load_test_run.id}`}>View performance results</Link></section>}

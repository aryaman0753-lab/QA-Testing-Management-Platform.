...
export function ApiRequestBuilder({ request, draftCollectionId, environmentId, canManage, canExecute, onSaved, onDeleted }: {
  ...
  return <div className="request-builder">
    <div className="request-toolbar"><input aria-label="Request name" className="request-name" value={form.name} onChange={(e) => set("name", e.target.value)} /><div className="request-actions">{request && <a className="btn btn-secondary link-button" href={`/projects/${request.project_id}/load-testing/create?request_id=${request.id}`}>Load test</a>}{request && canManage && <Button variant="danger" onClick={remove}>Delete</Button>}{canManage && <Button variant="secondary" isLoading={saving} onClick={() => void save()}>Save</Button>}{canExecute && <Button isLoading={sending} onClick={send}>Send</Button>}</div></div>
